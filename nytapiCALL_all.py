    ...
    # try:
        ...
        while d < d_end:
            ...
            try:
                ...
                error_value = [d.strftime("%Y-%m-%d"),offset]
                print "------error in total--------"
                print error_value
                insert_query = "INSERT INTO error_log ( error_date, offset) " \
                                   " VALUES( '%s', '%s')" % \
                                   (d.strftime("%Y-%m-%d"),offset)
                ...
                if "comments" in r["results"]:
                    for comment in r["results"]["comments"]:
                        ...
                        try:
                            ...
                            commentBody = str(comment["commentBody"])
                        ...
                        updateDate = int(comment["updateDate"])
                        ...
                        insert_query = "INSERT INTO nyt_comments_all ( statusID, replyCount, " \
                                       "parentID, commentType, commentID, status, " \
                                       "commentSequence, commentBody," \
                                       "createDate, updateDate, approveDate," \
                                       "recommendationCount, editorsSelection, display_name, userFirstName, gender," \
                                       " userID,location, articleURL, articleID, lft, rgt)" \
                                       " VALUES( %d, %d,'%s', '%s', %d, '%s', %d, '%s', " \
                                       "FROM_UNIXTIME(%d),FROM_UNIXTIME(%d),FROM_UNIXTIME(%d), " \
                                       "%d, %d, '%s', '%s', '%s', %d, '%s', '%s', %d, '%s', '%s')" % \
                                       (statusID, replyCount, parentID,commentType.decode("utf8"),
                                        commentID, status.decode("utf8"),
                                        commentSequence, commentBody.decode("utf8"), createDate, updateDate, approveDate,
                                        recommendationCount, editorsSelection, display_name.decode("utf8"),
                                        userFirstName.decode("utf8"), userGender ,userID,
                                        location.decode("utf8"), articleURL.decode("utf8"), articleID, lft, rgt)
