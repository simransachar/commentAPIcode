...
#    for c in comments:
    ...
    for row in cursor:
        ...
        if n % 100 == 0 :
            print n
#        ct = CleanAndTokenize(comments[c][2])
...
#        print str(i) + " " + str(key) + " " + "and value is: " + str(val)
        if word_freq < 10:
            unigram_cutoff = i - 1
            break;
...
#    word_features.extend(fd.keys()[:unigram_cutoff])
    ...
    # fileWriter = csv.writer(open("data/vocab88.csv", "wb"),delimiter=",")
    # for w in fd.most_common(unigram_cutoff):
    #     row = [w[0].encode("utf8"), w[1]]
    #     fileWriter.writerow(row)
...
def ComputeCommentArticleRelevance():
    ...
    csvFile = open("data/vocab90.csv", 'Ur')
    csvReader = csv.reader(csvFile, delimiter=',', quotechar='"')
...
#    for row in csvReader:
 #       if csvReader.line_num > 1:
    ...
    fileWriter = csv.writer(open("data/comment_study_article_relevance3.csv", "wb"),delimiter=",")
       ...
       # print article_stemmed_tokens
        for w in vocab_freq:
            ...
            if w in article_stemmed_tokens:
                 article_features[w] = article_stemmed_tokens_fd[w] * math.log(Decimal(nDocuments) / Decimal(df))
            else:
                 article_features[w] = 0.0
            if w in comment_stemmed_tokens:
                 comment_features[w] = comment_stemmed_tokens_fd[w] * math.log(Decimal(nDocuments) / Decimal(df))
            else:
...
#                ct = CleanAndTokenize(comment[2].decode("utf8"))
                ...
                for w in vocab_freq:
                    centroid_comment_features[w] = centroid_comment_stemmed_tokens_fd[w] * math.log(Decimal(nDocuments) / Decimal(vocab_freq[w]))
