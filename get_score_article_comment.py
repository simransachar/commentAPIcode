...
csvFile = open("articleData_bkup.csv", 'Ur')
csvReader = csv.reader(csvFile, delimiter=',', quotechar='"')

url = "http://127.0.0.1:5000/commentIQ/v1/addComment"

fileWriter = csv.writer(open("articleData_withScores.csv", "ab"),delimiter=",")
...
for row in csvReader:
    if csvReader.line_num == 1:
        ...
        header = ["userID","status","commentBody","approveDate","recommendationCount", \
                            ...
                            "PersonalXP","Readability","Brevity","RecommendationScore"]
#        fileWriter.writerow(header)
    else:
        commentBody = row[2]
        params = {'commentBody' : commentBody, 'articleID' : 123, 'RecommendationCount' : row[4] }
        param_json = json.dumps(params)
        response = requests.post(url, param_json)
        r = response.json()
        print r
        scores = [r['ArticleRelevance'],r['ConversationalRelevance'],r['PersonalXP'],r['Readability'], \
                          r['Brevity'],r['RecommendationScore']]
        data = row + scores
