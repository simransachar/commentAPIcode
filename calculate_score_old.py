...
def calRecommendationScore(Recommendationcount):
    Recommendationcount = int(Recommendationcount)
    if Recommendationcount > 1:
        R_score = math.log(Decimal(Recommendationcount))
    else:
        R_score = 0.0
    return R_score
...
def addComment(comment_text,articleID,RecommendationCount):
    operation = "add"
    ...
    RecommendationScore = calRecommendationScore(RecommendationCount)
    return (ArticleRelevance,ConversationalRelevance,PersonalXP,Readability,Brevity,RecommendationScore)
