...
cursor.execute("select distinct(userID) from vocab_comments where userID is not null and "
               "createDate between  '2015-01-01' and  '2015-02-01'")
for ids in cursor:
    userID_list.append(ids)
...
def CollectComments():
    # try:
        ...
        for ID in userID_list:
            print ID
            cursor.execute("select count(*) from user_comments where user_id = '"+ str(ID[0]) +"'")
            for c in cursor:
                print c
                if c[0] == 0:
